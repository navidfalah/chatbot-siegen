...
class RAGEnhancedSocialHealthAI:
    ...
    def understand_problem(self, question: str, chat_history: List[ChatMessage], language: str) -> Dict[str, any]:
        ...
        if chat_history:
            ...
            if language.startswith("de"):
                history_context = "Bisheriger Verlauf zu sozialen/gesundheitlichen Themen:\n"
                for msg in recent_messages:
                    history_context += f"F: {msg.question}\nA: {msg.answer[:150]}...\n"
            else:
    ...
    def is_question_in_topic(self, question: str, chat_history: List[ChatMessage], language: str) -> Tuple[bool, str]:
        ...
        if chat_history:
            ...
            if language == "de":
                history_context = "Vorheriger Gesprächsverlauf:\n"
                for msg in recent_messages:
                    history_context += f"F: {msg.question}\nA: {msg.answer[:100]}...\n"
            else:
    ...
    def generate_rag_enhanced_response(self, question: str, understanding: Dict, chat_history: List[ChatMessage], rag_results: List[RAGResult], rag_context: str, language: str) -> str:
        ...
        if chat_history and understanding['requires_context']:
            if language == "de":
                conversation_context = "Vorheriger Gesprächsverlauf:\n"
                for msg in chat_history[-2:]:
                    conversation_context += f"F: {msg.question}\nA: {msg.answer[:200]}...\n\n"
            else:
                conversation_context = "Previous conversation context:\n"
                for msg in chat_history[-2:]:
                    conversation_context += f"Q: {msg.question}\nA: {msg.answer[:200]}...\n\n"
        if language == "de":
            main_prompt = f"Hilf dem Nutzer lokale Angebote für folgendes zu finden: {question}"
        else:
            main_prompt = f"Help the user find local services for: {question}"
        # Other intents omitted for brevity, but adapt as in original code for all mappings
        full_context = (
            f"{self.get_system_context(language)}\n\n"
            f"{rag_context}\n\n"
            f"{conversation_context}\n\n"
            f"Wichtige Hinweise:\n" if language == "de" else "Important Instructions:\n"
            f"- {'Nutze die lokalen Informationen oben, wenn relevant.' if language == 'de' else 'Use the local services information provided above when relevant'}\n"
            f"- {'Immer Kontaktdaten angeben (Adresse, Telefon, E-Mail, Webseite), wenn verfügbar.' if language == 'de' else 'Always include contact information (address, phone, email, website) when available'}\n"
            f"- {'Lokale Angebote aus Siegen bevorzugen.' if language == 'de' else 'Prioritize local Siegen resources from the database'}\n"
            f"- {'Nur unterstützen und keine medizinischen Diagnosen stellen.' if language == 'de' else 'Include appropriate medical disclaimers when needed'}\n"
            f"- {'Sei empathisch und unterstützend.' if language == 'de' else 'Be supportive and empathetic'}\n"
            f"- {'Praktische und umsetzbare Empfehlungen geben.' if language == 'de' else 'Provide practical, actionable advice'}\n\n"
            f"Nutzerfrage: {question}\n\n"
            f"AUFGABE: {main_prompt}\n\n"
            f"{'Bitte antworte umfassend und binde die lokalen Ressourcen ggf. ein.' if language == 'de' else 'Please provide a comprehensive response that incorporates the local services information where relevant.'}"
        )
        return self.ollama.generate_response(full_context, language=language)

    def ask_for_clarification(self, understanding: Dict, language: str) -> str:
        if language == "de":
            prompt = (
                f"{self.get_system_context(language)}\n\n"
            ...
            try:
                ...
                if is_in_topic:
                    ...
                    if self.ai.do_we_have_data(understanding, self.chat_history, rag_results, language):
                        ...
                        clarification_request = self.ai.ask_for_clarification(understanding, language)
